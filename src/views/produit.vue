  ...
  <div class="min-h-screen bg-gray-50 p-4 md:p-6">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="bg-white shadow-md rounded-lg p-4 mb-6">
        <div class="relative">
          <input v-model="searchQuery" type="text" placeholder="🔍 Rechercher un produit par nom, prix ou catégorie..."
      ...
      <form @submit.prevent="handleSubmit" class="bg-white shadow-md rounded-lg p-4 md:p-6 mb-6">
      ...
      <div v-else class="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <div class="block md:hidden">
          <div v-for="(produit, index) in filteredProduits" :key="produit.id" class="border-b last:border-b-0 p-4">
            <div class="flex gap-4">
              <img :src="produit.photo || '/images/default-product.png'" alt="" class="w-20 h-20 object-cover rounded"
                @error="handleImageError($event)" />
              <div class="flex-1">
                <h3 class="font-semibold text-lg">{{ produit.nom }}</h3>
                <p class="text-blue-600 font-bold">{{ formatPrice(produit.prix) }} FCFA</p>
                <span v-if="produit.categorie"
                  class="inline-block mt-1 px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
          ...
          <table class="min-w-full">
            ...
            <tbody>
              <tr v-for="(produit, index) in filteredProduits" :key="produit.id" class="border-t hover:bg-gray-50">
                <td class="px-4 py-3">
                  <img :src="produit.photo || '/images/default-product.png'" alt=""
                    class="w-16 h-16 object-cover rounded" @error="handleImageError($event)" />
                </td>
...
async function createProduct() {
  ...
  try {
    ...
    console.log('erreur de code produit:', data, form.value.categorie_id)
