<template>
  ...
  <!-- NAVBAR FIXE -->
  <nav class="w-full h-[70px] md:h-[80px] text-white bg-[#000A2B] fixed top-0 left-0 z-50 shadow-lg">
    <div class="h-full max-w-screen-2xl mx-auto px-4 md:px-6 lg:px-8 flex justify-between items-center">
      ...
      <div class="hidden md:flex items-center gap-4 lg:gap-6">
        ...
        <button @click="toggleDetailPanier" class="relative hover:text-amber-400 transition-colors">
          ...
          <span 
            v-if="CartStore.produitsPanier?.length > 0"
      ...
      <div class="absolute right-0 top-0 bottom-0 w-4/5 max-w-sm bg-[#000A2B] shadow-2xl overflow-y-auto">
        <div class="p-6 space-y-6">
          ...
          <div class="flex items-center gap-3 pb-4 border-b border-white/10">
            <div class="w-12 h-12 bg-amber-400 rounded-full flex items-center justify-center text-[#000A2B] font-bold text-lg">
          ...
          <nav class="space-y-2">
            ...
            <button 
              @click="toggleDetailPanier(); toggleMobileMenu()"
  ...
  <main class="pt-[130px] lg:pt-[100px] px-4 pb-8 min-h-screen bg-gray-50">
    <div class="max-w-screen-2xl mx-auto">
      ...
      > 
        <Divs 
          v-for="product in produitsFiltres" 
          :key="product.id" 
          :product="product" 
  ...
  <Transition name="slide">
    <div v-if="isCategoriesOpen" class="fixed inset-0 z-50 flex">
      <Categorie 
        @close-categorie="toggleCategories" 
        @categorie-chosen="filtrerParCategorie" 
      />
      <div 
        @click="toggleCategories" 
...
async function filtrerParCategorie(categorie) {
  ...
  try {
    ...
    // Option 2 : Filtrer côté serveur (si votre API le permet)
    // const response = await axios.get(`http://127.0.0.1:8000/api/produits?categorie_id=${categorie.id}`)
    // produitsFiltres.value = response.data
